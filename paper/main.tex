...
\emph{side channel}, so band and cutoff $k$ are the only quantities that vary across arms, and we add a model-free probe measuring how much of the target each condition gives away. On SPECTRE's Planar benchmark, a sweep of five conditioning arms over five cutoffs shows no
frequency gradient: 23 of 25 cells are indistinguishable from the unconditioned baseline, and the
effect is confined to two isolated minima, which the probe then separates. Low-frequency conditioning at $k{=}8$
improves the MMD Ratio $2.3\times$ while recovering $0.0\%$ of the target's edges; high-frequency
conditioning at $k{=}32$ improves it $29.7\times$ but recovers $98.5\%$, making it a near-oracle
...
metric-free and orders the arms the same way Ratio does.}
\label{tab:k8}
\end{table}
...
is supplied. It does not (Figure~\ref{fig:sweep}(a)). Over the 23 conditioned cells that are not one of the two minima, mean Ratio is $327.2 \pm 18.5$,
and the unconditioned baseline of $335.7$ sits $+0.46$ standard deviations \emph{inside} that
spread: the flat region simply is the baseline. Against it the minima sit $-11.4$ (\texttt{low}, $k{=}8$) and $-17.1$
(\texttt{high}, $k{=}32$) standard deviations below, while their neighbours in the same band show
nothing --- \texttt{low} is $316.4$ at $k{=}4$ and $270.5$ at $k{=}16$, against $116.3$ at
$k{=}8$.
...
graph's own spectrum at a particular band and cutoff, which is the cleanest evidence we have that
...
Both reproduce; only one of them is leak-free.}
\label{tab:spikes}
\end{table}
...
reports $0\%$ validity at every scale. We therefore also report distance-to-planarity, which separates arms the boolean cannot:
\texttt{high}, $k{=}32$ has median distance 4, where \texttt{high}, $k{=}16$ and \texttt{low},
$k{=}32$ need over 12 edge removals in every case.
...
layers, 500 steps, 1{,}000 epochs) the \emph{unconditioned} baseline improves ${\sim}8\times$ over the pilot, from ${\sim}327$ to
$39.60$; conditioning still helps, to $15.40$ ($-61\%$), but the gap narrows from ${\sim}220$
Ratio points to ${\sim}24$. A headline from a small budget overstates conditioning's contribution, and validity stays $0\%$ at
every scale.
...
unconditioned augmentation actively hurts ($89.8\%$ against $94.8\%$ real-only) and conditioned
augmentation is no better ($92.0\%$); at $n{=}80$ both stay below the $97.2\%$ baseline. The control arm is informative: fresh samples from the \emph{true} SBM generator do help slightly
($98.8\%$ at $n{=}80$), which shows the pipeline works and locates the failure in sample quality. The task is also easy to begin with, so this shows the gains of \S\ref{sec:sweep}
...
hard to explain as anything but coarse global structure being useful and hard to infer alone.
...
top-line claim is our best argument for the information floor none of the cited work reports.

\paragraph{Two caveats on our own reporting.} First, the sharpness is unexplained: both minima reproduce eleven to seventeen standard deviations
below a flat surface, so they are real, but a mechanism predicting \emph{which} cells spike is
...
\paragraph{Limitations.} (i) Every number is validation-split, on splits that also selected checkpoints, and the quality
sweep is Planar only --- on SBM we ran the leakage half, which replicates, but not the generation
half. (ii) The sweep's values were transcribed from a recycled Colab session's stdout, so
...
a combinatorial Laplacian, SignNet, and a memorization audit --- were not run.

\section{Conclusion}
Measured as a controlled side channel with matched-capacity controls and an information floor: no
frequency gradient, one genuine low-frequency effect at a single cutoff, and a most-impressive
number the floor exposed as leakage.
